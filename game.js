(() => {
  ...
  function showDialog(speaker, lines, callback = null) {
    ...
    $("#actionButton small").textContent = "つぎへ";
  ...
  function advanceDialog(initial = false) {
    ...
    $("#dialogWindow").classList.add("hidden");
    $("#actionButton small").textContent = "はなす";
    mode = "field";
  ...
  function bindEvents() {
    ...
    $("#dialogWindow").addEventListener("pointerdown", (event) => {
    ...
    let moveRepeatDelay = null;
    let moveRepeatTimer = null;
    const stopTouchMove = () => {
      clearTimeout(moveRepeatDelay);
      clearInterval(moveRepeatTimer);
      moveRepeatDelay = null;
    ...
    $$(".dpad button").forEach((button) => {
      ...
      button.addEventListener("pointerdown", (event) => {
        ...
        moveRepeatDelay = setTimeout(() => {
          moveRepeatTimer = setInterval(() => move(...d), 125);
        }, 280);
      });
      ["pointerup", "pointercancel", "lostpointercapture"].forEach((eventName) => {
        button.addEventListener(eventName, stopTouchMove);
      });
    });
